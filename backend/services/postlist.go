...
func PostListHandler(msg []byte, urlClient string, callback func(response Response)) {
	...
	go func() {
		...
		user, err := secMod.UserManagement.FindUserByToken(requestData.TokenSesion)
		if err != nil {
		...
		mensajes, err := secMod.RoomManagement.GetMessagesFromId(idSalaUUID, idmensaje)
		if err != nil {
